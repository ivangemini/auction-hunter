...
const outputPath = path.join(outputRoot, 'p7-item-art-batch-01.png');
...
try {
  ...
  try {
    ...
    const cards = ITEM_IDS.map((id, index) => `
      <article class="card">
    ...
    await page.setContent(`<!doctype html>
      <html>
        ...
        <body>
          <main>
            <header><h1>P7 · Authored Item Art — Batch 01</h1><div class="meta">9 SEMANTIC IDS · 512×360 VECTOR SOURCE</div></header>
    ...
    assert(screenshot.length > 80_000, `Item-art review screenshot looks unexpectedly small (${screenshot.length} bytes)`);
    assert(screenshot.subarray(12, 16).toString('ascii') === 'IHDR', 'Item-art review output is not a PNG');
    assert(screenshot.readUInt32BE(16) === 1280 && screenshot.readUInt32BE(20) === 720, 'Item-art review must be 1280x720');
    await page.close();
  } finally {
    await browser.close();
  }
...
console.log('P7 item-art visual review capture OK');
