...
import {
  CAMPAIGN_BREADTH_THREE_ITEMS,
  CAMPAIGN_BREADTH_THREE_ROUTES,
  CAMPAIGN_BREADTH_THREE_SETS,
  registerCampaignBreadthThree,
} from './campaignBreadthThree';
import { ITEM_BY_ID, ITEMS } from './catalog';
import { ALL_LOTS } from './catalogBreadth';
import { COLLECTION_SETS } from './collections';

describe('P9 third campaign catalog batch', () => {
  it('ships six bilingual identities with direct art', () => {
    expect(CAMPAIGN_BREADTH_THREE_ITEMS).toHaveLength(6);
  ...
  it('routes every identity through at least two truthful normal-auction lots', () => {
    for (const item of CAMPAIGN_BREADTH_THREE_ITEMS) {
      const routes = CAMPAIGN_BREADTH_THREE_ROUTES[item.id] ?? [];
  ...
  it('adds two rewarding collection goals covering all six additions', () => {
    const setIds = new Set(CAMPAIGN_BREADTH_THREE_SETS.map((set) => set.id));
    ...
    for (const item of CAMPAIGN_BREADTH_THREE_ITEMS) expect(covered.has(item.id), item.id).toBe(true);
  });
