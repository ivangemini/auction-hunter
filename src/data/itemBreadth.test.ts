...
import {
  BREADTH_COLLECTION_SETS,
  BREADTH_ITEMS,
  BREADTH_LOT_ROUTES,
  registerItemBreadth,
} from './itemBreadth';

describe('second-wave item breadth', () => {
  it('installs six stable bilingual items with direct art', () => {
    expect(BREADTH_ITEMS).toHaveLength(6);
    expect(new Set(BREADTH_ITEMS.map((item) => item.id)).size).toBe(6);
  ...
  it('is idempotent across repeated bootstrap/test installation', () => {
    const before = {
      ...
      pools: Object.fromEntries(
        Object.entries(BREADTH_LOT_ROUTES).map(([itemId, lotIds]) => [
          itemId,
